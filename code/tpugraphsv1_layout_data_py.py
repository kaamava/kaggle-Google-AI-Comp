...
# 
# https://github.com/google-research-datasets/tpu_graphs/blob/main/tpu_graphs/baselines/layout/data.py
...
def get_npz_split(
  ...
  def pickleload(file):
    ...
    ff.close()
    return obj
  tmpname = split_path.split('\\')[-1]
  picklesave(files,f'{tmpname}.pkl')
  ...
  if cache_filename and tf.io.gfile.exists(cache_filename):
    ...
    for filename in tqdm.tqdm(files):
      try:
        ...
        npz_dataset.add_npz_file(graph_id, np_data, min_configs=min_configs, max_configs=max_configs)
      except:
        print(filename)
...
def get_npz_dataset(
  ...
  npz_dataset = NpzDataset(
      train=get_npz_split(
          os.path.join(root_path, 'train_split'), cache_dir=cache_dir,
          min_configs=min_train_configs, max_configs=max_train_configs),
      validation=get_npz_split(
          os.path.join(root_path, 'valid_split'), cache_dir=cache_dir,
          min_configs=min_train_configs, max_configs=max_train_configs),
      test=get_npz_split(
